...
# Импорты для обработки документов (без storage)
try:
    ...
    logging.warning(f"Extractor недоступен: {e}")
...
class DocumentProcessor:
    """Асинхронный процессор документов (автономная версия)"""
    ...
    async def _process_task(self, task: ProcessingTask, worker_name: str):
        ...
        try:
            if not EXTRACTOR_AVAILABLE:
                # Симуляция обработки для тестирования
                ...
                ordered_fields = {
                    ...
                    'date': '2024-01-15',
                    'amount': '100000.00',
                    'subject': 'Тестовые услуги',
                    'contract_number': 'Д-2024-001'
                }
                
                # Валидируем данные (если доступен валидатор)
...
⏱️ **Время обработки:** {(task.completed_at - start_time).total_seconds():.1f} сек
                ...
                # Реальная обработка с extractor
                ...
                # Сохраняем документ в базу данных (если доступна)
                try:
